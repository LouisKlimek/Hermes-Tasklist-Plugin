...
Each node shows its **one real status** — the dot and the label agree (both use the live Kanban colour/label), so there's never a “blocked dot but Ready text” contradiction. A task waiting on an unfinished parent keeps a “waiting on a parent” note, while only tasks whose status is `blocked` get a **bold pulsing red outline**. **Jump between blocked tasks:** the legend shows ◀ Blocked k/n ▶ controls that smoothly pan+zoom the graph from one blocked task to the next. Click a node to open that task; hover to trace its full ancestor/descendant
...
- **Follow‑up from feedback** — tasks that are **Done** get a **Follow‑up** button (in the detail popup header *and* on the list row for quick access). It opens a small popup with a **Feedback** textarea, optional **Attachments** (add/remove files before submitting), an **Assignee** selector (searchable list of every board profile, exactly like the New‑task popup, defaulting to the source task's own assignee) and a **Priority** selector (same Urgent / High / Normal / Low options and colors as the New‑task popup). On submit it auto‑creates a `Follow-up: …` card of type *Post‑Merge Feedback* with status **todo**, prefilled with the original task id and (when detected in the task body) the original pull‑request URL, assigned to the chosen profile, and **linked as a child of the original task** so the relationship is preserved. Files use the host Kanban attachment service, which performs authorization and file validation; attachments then appear in the follow-up task's normal protected attachment area.
...
- Inside an open list, each status section has a **+ Add task** row that creates a new task on that board in that list and status. List membership remains structured TaskList data; the shared Kanban title stays exactly as entered.
