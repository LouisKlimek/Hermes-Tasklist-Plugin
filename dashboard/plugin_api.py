"""TaskList dashboard plugin — backend for custom (ClickUp-style) lists.

Stores user-defined named lists and per-task membership in its OWN SQLite
database, independent of kanban.db. Lists are a human organizational overlay:
agents, the CLI and workers don't see them. Membership is keyed by the kanban
task id and scoped by board slug, so it mirrors the kanban board switcher.

The dashboard plugin system imports this module and mounts ``router`` at
``/api/plugins/tasklist/``. On a loopback-bound dashboard these routes inherit
the same localhost trust boundary as the bundled kanban plugin's API.

DB location: ``$HERMES_HOME/tasklist/lists.db`` (default ``~/.hermes/...``).
"""
...
def get_lists(board: Optional[str] = Query(None)):
    """Return every list for the board plus the task_id -> list_id map."""
    b = _board(board)
    c = _conn()
    try:
        lists = [
...
def create_list(body: ListCreate, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        return {
            "list": {
                "id": lid, "name": name, "color": body.color,
...
def patch_list(list_id: str, body: ListPatch, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        cur = c.execute(
            f"UPDATE lists SET {', '.join(sets)} WHERE id=? AND board=?", vals
        )
        c.commit()
        if cur.rowcount == 0:
...
def delete_list(list_id: str, board: Optional[str] = Query(None)):
    ...
    try:
        c.execute(
            "DELETE FROM membership WHERE board=? AND list_id=?", (b, list_id)
        )
        cur = c.execute(
            "DELETE FROM lists WHERE id=? AND board=?", (list_id, b)
        )
        c.commit()
        if cur.rowcount == 0:
            raise HTTPException(status_code=404, detail="list not found")
...
def set_membership(body: MembershipBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if body.list_id:
            ...
            c.execute(
                "DELETE FROM membership WHERE board=? AND task_id=?",
                (b, body.task_id),
            )
        c.commit()
        return {"ok": True}
