...
- **Follow‑up from feedback** — tasks that are **Done** get a **Follow‑up** button (in the detail popup header *and* on the list row for quick access). It opens a small popup with a **Feedback** textarea, a **Route** selector (Auto / Developer / Designer / CEO / QA) and a **Priority** selector. On submit it auto‑creates a `Follow-up: …` card of type *Post‑Merge Feedback*, prefilled with the original task id and (when detected in the task body) the original pull‑request URL, routed to the chosen assignee, and **linked as a child of the original task** so the relationship is preserved.
