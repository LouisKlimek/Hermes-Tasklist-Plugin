...
- **Mobile‑friendly popups** — popups (task detail, New‑task, file preview) are sized for phones, and the phone's hardware/gesture **Back button just closes the topmost open popup** instead of navigating the dashboard away from the page. Back keeps peeling layers (file preview → back‑stack, then the popup) until everything is closed, then behaves normally.
