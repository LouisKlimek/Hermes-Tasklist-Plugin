...
- Inside an open list, each status section has a **+ Add task** row that creates a new task on that board in that list and status. The shared Kanban title retains its origin as `Task title [List name]` for agents and the native board; TaskList itself displays the human-readable title without that generated trailing suffix.
